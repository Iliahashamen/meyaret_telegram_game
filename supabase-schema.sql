...
-- ── Add last_drop_at for DROP DA BALL (run once on existing DB) ──────────────
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name='users' AND column_name='last_drop_at'
  ) THEN
    ALTER TABLE users ADD COLUMN last_drop_at TIMESTAMPTZ;
  END IF;
END $$;
